...
#define GAS_PIN  A1   // MQ Gas Sensor
#define HEART_PIN A2  // Potentiometer as Heart Rate
#define BTN_PIN 7     // GPS button
...
// GPS simulated coordinates
const char* gpsLocations[] = {
  "19.0760N,72.8777E",   // Mumbai
  "28.6139N,77.2090E",   // Delhi
  "13.0827N,80.2707E",   // Chennai
  "26.9124N,75.7873E",   // Jaipur
  "22.5726N,88.3639E"    // Kolkata
};
int gpsIndex = 0;
...
// Read Gas Sensor (MQ-4 style)
int readGas() {
  return analogRead(GAS_PIN); // directly read analog value
}
...
void loop() {
  ...
  // ================= GPS switching (manual + auto) =================
  if (digitalRead(BTN_PIN) == LOW) {
    // Manual: button press → next location
    if (millis() - lastGpsChange > debounce) {
      gpsIndex = (gpsIndex + 1) % gpsCount;
      lastGpsChange = millis();
    }
  } else {
    // Automatic: change every 10 sec
